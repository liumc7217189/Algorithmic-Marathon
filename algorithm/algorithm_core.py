# -*- coding: utf-8 -*-
"""
Created on Fri Dec 14 09:45:35 2018
...
def my_strategy(input_data):
    ...
    # 用回家需要的步子来算，是不是要回家了
    #coord = {}
    # go_home(x,y,home_x,home_y,walls):
    step_num,act_list=go_home(my_state['x'],my_state['y'],my_state['home_x'],my_state['home_y'],walls)
    #print('step_num:',step_num,'act_list:',act_list)
    if (total_step - cur_step) <= int(step_num) :
        # 先判断是不是墙
...
def udlrs(now_coord,jobs,walls):
    ...
    for loc in list(['U','D','L','R','S']):
        if int(loc_coord[loc]['x']) > 11 or int(loc_coord[loc]['x']) < 0 or int(loc_coord[loc]['y']) > 11 or int(loc_coord[loc]['y']) < 0:
            loc_coord[loc] = -100
    for loc in list(['U','D','L','R','S']):
        if walls[int(loc_coord[loc]['x']),int(loc_coord[loc]['y'])]==0: #
            loc_value[loc] = -100
...
def go_home(x,y,home_x,home_y,walls):
    ...
    return(step_num,act_list)

"""
param = {
        'player1':{'name': 'p1', 'x': 3, 'y': 3, 'home_x': 5, 'home_y': 5, 'n_jobs': 0, 'value': 0, 'score': 0},
        'player2':{'name': 'p2', 'x': 6, 'y': 6, 'home_x': 6, 'home_y': 6, 'n_jobs': 0, 'value': 0, 'score': 0},
...
print(algorithm_main(param, user='player1', curStep=10, totalStep=200))
